...
\begin{enumerate}
    ...
    \item \textbf{Hỗ Trợ Đa Thuật Toán Toàn Diện}: Không giống như các nền tảng thương mại giới hạn ở mạng nơ-ron hoặc các thuật toán ML chuyên biệt, framework của chúng tôi tích hợp ba phương pháp bổ sung (Random Forest, SVM, Neural Networks) với các API nhất quán và tối ưu hóa siêu tham số tự động. Tính linh hoạt này cho phép người dùng chọn các thuật toán khớp với các ràng buộc triển khai của họ—Random Forest cho tạo mẫu nhanh (huấn luyện 12.3s), Neural Networks cho độ chính xác tối đa (96.2\%), hoặc SVM cho khả năng diễn giải lý thuyết.

    \item \textbf{Tạo Mã Nhận Biết Tối Ưu Hóa}: Kiến trúc trình tạo modular tạo ra các triển khai C++ đặc thù nền tảng với bốn hồ sơ tối ưu hóa (accuracy, speed, power, balanced). Mã được tạo đạt độ trễ suy luận 12-18ms và dấu chân bộ nhớ 95-182KB trên vi điều khiển 64MHz—hiệu suất cạnh tranh với các triển khai thủ công chuyên gia trong khi yêu cầu không có chuyên môn hệ thống nhúng.

    \item \textbf{Xử Lý Mất Cân Bằng Lớp}: Bằng cách làm cho \texttt{class\_weight='balanced'} là một cấu hình mặc định thay vì tham số tùy chọn, framework hệ thống giải quyết một thách thức quan trọng nhưng thường bị bỏ qua trong theo dõi chuyển động thực tế. Các thử nghiệm của chúng tôi chứng minh cải thiện +42-47\% trong recall lớp thiểu số, chuyển đổi các mô hình từ các nguyên mẫu nghiên cứu thành các hệ thống sẵn sàng triển khai.

    \item \textbf{Đảm Bảo Tương Đồng Huấn Luyện-Triển Khai}: Chúng tôi xác định và giải quyết một thách thức ít được nghiên cứu trong edge ML: sự không khớp giữa pipeline trích xuất đặc trưng huấn luyện (Python) và suy luận (C++). Quy trình xác minh 12 điểm kiểm tra đảm bảo tính nhất quán bit-exact cho các đặc trưng miền thời gian, bao gồm sửa lỗi zero-padding, thống nhất công thức kurtosis/skewness, và reorder đặc trưng tự động tại thời điểm tạo mã. Phát hiện này nhấn mạnh rằng tính minh bạch không chỉ là triết lý mã nguồn mở mà là yêu cầu kỹ thuật thiết yếu cho triển khai edge ML đáng tin cậy \cite{paleyes2022challenges}.
...
Các thử nghiệm toàn diện sử dụng tập dữ liệu Nhận Dạng Hoạt Động Con Người sáu lớp xác thực hiệu quả của framework:
\begin{itemize}
    ...
    \item \textbf{Tính Khả Thi Thời Gian Thực}: Suy luận 9-18ms trên ARM Cortex-M4 @ 64MHz với tiêu thụ điện năng 30mW, cho phép hoạt động pin liên tục 65+ giờ
...
\begin{enumerate}
    ...
    \item \textbf{Tạo Mã Không Phụ Thuộc Nền Tảng} \checkmark: Kiến trúc trình tạo modular được xác thực trên ARM Cortex-M4 với bốn chế độ tối ưu hóa và thiết kế có thể mở rộng hỗ trợ các nền tảng tương lai
    
    \item \textbf{Xác Thực Hiệu Quả Framework} \checkmark: Đánh giá toàn diện chứng minh độ chính xác cạnh tranh (96.2\%), hiệu suất thời gian thực (suy luận 12ms) và sự vượt trội so với các lựa chọn thay thế thương mại trên các số liệu chi phí-hiệu suất
...
Xác thực hiện tại sử dụng dữ liệu đối tượng đơn cho sáu hoạt động. Công việc tương lai nên:
\begin{itemize}
...
\begin{itemize}
    \item Xác thực tạo mã trên AVR (Arduino Uno), RISC-V (ESP32-C3), ARM Cortex-M0+ (Raspberry Pi Pico)
...
Mã nguồn, tài liệu và các tập dữ liệu ví dụ có sẵn công khai để hỗ trợ khả năng tái tạo, giáo dục và đóng góp cộng đồng. Chúng tôi mời các nhà nghiên cứu và nhà thực hành xây dựng trên nền tảng này, mở rộng khả năng của framework và áp dụng nó cho các thách thức theo dõi chuyển động mới lạ. Cùng nhau, chúng ta có thể bắc cầu khoảng cách giữa nghiên cứu phòng thí nghiệm và triển khai thực tế, chuyển đổi bối cảnh phát triển edge AI.
