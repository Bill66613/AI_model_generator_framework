...
Các bộ lọc IIR Butterworth truyền thống gặp hạn chế parity cố hữu: \texttt{filtfilt} (non-causal, zero-phase) trong huấn luyện xử lý dữ liệu theo cả hai chiều, trong khi \texttt{lfilter} (causal, forward-only) trong triển khai chỉ xử lý theo một chiều do ràng buộc thời gian thực. Khoảng cách này gây sai lệch đặc biệt nghiêm trọng với các đặc trưng nhạy cảm với pha như skewness và kurtosis.
