...
\subsection{Tiền xử lý tương tác: một bước ngoặt về mô hình}
Giao diện kéo thả cửa sổ thời gian đại diện cho một bước ngoặt so với quy trình tiền xử lý truyền thống. Các phương pháp thông thường yêu cầu người dùng hoặc viết mã để phân đoạn dữ liệu (rào cản kỹ thuật cao), hoặc dùng phân đoạn tự động kiểu hộp đen (thiếu kiểm soát, dễ sai). Giao diện kéo thả loại bỏ các điểm khó khăn này bằng cách cho phép thao tác trực tiếp trên biểu đồ tín hiệu, giảm thời gian tiền xử lý ước tính 60--80\% so với chú thích thủ công bằng nhãn thời gian.
...
Chi phí phát triển: Framework cần triển khai logic suy luận riêng cho mỗi thuật toán ($\sim$1580 dòng mã cho trích xuất đặc trưng dùng chung, mỗi generator đặc thù thêm 200--500 dòng). Tuy nhiên, đây là \textit{chi phí một lần}: sau khi generator được kiểm chứng cho một thuật toán, nó hoạt động đúng cho mọi mô hình được huấn luyện bằng thuật toán đó.
...
Các bộ lọc IIR Butterworth truyền thống tồn tại vấn đề parity vốn có: \texttt{filtfilt} (non-causal, zero-phase) trong huấn luyện xử lý dữ liệu theo cả hai chiều, trong khi \texttt{lfilter} (causal, forward-only) trong triển khai chỉ có thể xử lý theo một chiều do ràng buộc thời gian thực. Khoảng cách này gây sai lệch đặc biệt nghiêm trọng với các đặc trưng nhạy cảm với pha như skewness và kurtosis.
...
Việc hỗ trợ đồng thời các mô hình feature-based (RF/SVM/NN) và end-to-end (CNN) trong cùng một framework đặt ra thách thức kiến trúc đặc thù. Case study về CNN metadata mismatch minh họa tầm quan trọng của architecture-aware code generation: mô hình CNN hiển thị tên file ``f33'' (33 features) dù thực tế sử dụng 6 channels per timestep. Logic fallback không phân biệt được giữa feature-based model (cần tên đặc trưng thống kê) và CNN model (cần placeholder cho từng channel). Giải pháp là sinh metadata nhận biết kiến trúc, với logic phân nhánh rõ ràng cho từng loại mô hình.

\subsection{Hạn chế TFLite và giải pháp Keras surrogate}
\label{sec:disc_tflite_limitations}

Pipeline chuyển đổi ONNX$\rightarrow$TFLite gặp vấn đề: \texttt{skl2onnx} tạo thành công ONNX-ML operators (TreeEnsembleClassifier, SVMClassifier), nhưng \texttt{onnx2tf} không hỗ trợ ONNX-ML operators---chỉ hỗ trợ standard ONNX operators. Kết quả: RF/SVM không thể chuyển đổi trực tiếp sang TFLite.
...
\begin{enumerate}
    ...
    \item \textbf{Khả năng tiếp cận} \checkmark: giao diện web với đường cong học tập thoải
