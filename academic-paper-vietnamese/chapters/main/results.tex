...
Các tham số chung: kích thước cửa sổ $W = 150$ mẫu (1,5 giây), bước trượt $S = 75$ mẫu (50\% chồng lấp), đệm cửa sổ bằng lặp giá trị biên (edge-value replication). Chia dữ liệu: 60\% huấn luyện, 20\% xác thực, 20\% kiểm tra, sử dụng phân chia phân tầng (stratified split). Loại đặc trưng sử dụng: \texttt{orientation\_invariant} (63 đặc trưng: thống kê thời gian trên acc\_mag, gyro\_mag, acc\_jerk\_mag cộng với đặc trưng phổ DFT trên acc\_mag và gyro\_mag); riêng CNN sử dụng đầu vào thô 6 trục ($150 \times 6$).
...
Các phép đo dưới đây được thực hiện trên Seeed XIAO nRF52840 (ARM Cortex-M4F @ 64~MHz, 1~MB flash, 256~KB RAM) như nền tảng tham chiếu đại diện cho lớp vi điều khiển wearable. Kết quả minh họa tính khả thi triển khai trên lớp phần cứng này, không đại diện cho toàn bộ ma trận đích.

% =====================================================================
% TODO: Thay thế bảng placeholder bên dưới bằng dữ liệu đo thực tế
% từ Arduino IDE compilation và kiểm tra trên thiết bị.
% Xem THESIS_DATA_INPUT.md Mục 2 và Mục 4 để biết định dạng cần điền.
% =====================================================================

\subsection{Sử dụng Bộ nhớ}
\label{sec:memory_usage}

Bảng~\ref{tab:memory_usage} báo cáo kích thước mã biên dịch từ Arduino IDE cho các mô hình đã huấn luyện trên bài toán 5 lớp.

\begin{table}[H]
    \centering
    \caption{Sử dụng bộ nhớ trên Seeed XIAO nRF52840 (1~MB Flash, 256~KB RAM) --- bài toán 5 lớp}
    \label{tab:memory_usage}
    \small
    \begin{tabular}{@{}lcccc@{}}
        \toprule
        Mô hình & Flash (bytes) & Flash (\%) & SRAM (bytes) & SRAM (\%) \\
        \midrule
        Neural Network & \textit{cần đo} & --- & \textit{cần đo} & --- \\
        Random Forest & \textit{cần đo} & --- & \textit{cần đo} & --- \\
        SVM (RBF) & \textit{cần đo} & --- & \textit{cần đo} & --- \\
        \bottomrule
    \end{tabular}
\end{table}

\subsection{Độ chính xác và Thời gian Suy luận Trên Thiết bị}
\label{sec:ondevice_accuracy}

Để đánh giá hiệu suất triển khai thực tế, các mô hình được nạp lên thiết bị và kiểm tra với từng hoạt động. Ngưỡng tin cậy $\tau = 0{,}6$ được sử dụng để từ chối các dự đoán không chắc chắn (trả về ``unknown'' thay vì ép nhãn).

\begin{table}[H]
    \centering
    \caption{Độ chính xác trên thiết bị --- bài toán 5 lớp, ngưỡng tin cậy $\tau = 0{,}6$}
    \label{tab:ondevice_accuracy}
    \small
    \begin{tabular}{@{}lcccc@{}}
        \toprule
        Mô hình & \makecell{Độ chính xác\\trên thiết bị (\%)} & \makecell{Tỷ lệ từ\\chối (\%)} & \makecell{Thời gian suy\\luận trung bình (ms)} & Ghi chú \\
        \midrule
        Neural Network & \textit{cần đo} & \textit{cần đo} & \textit{cần đo} & --- \\
        Random Forest & \textit{cần đo} & \textit{cần đo} & \textit{cần đo} & --- \\
        SVM (RBF) & \textit{cần đo} & \textit{cần đo} & \textit{cần đo} & --- \\
        \bottomrule
    \end{tabular}
\end{table}

\textbf{Dữ liệu tham khảo (bài toán 3 lớp)}: Mô hình PyTorch 1D-CNN đạt độ chính xác triển khai 99,4\% với tỷ lệ từ chối 0,6\% và độ tin cậy trung bình 98,8\% --- cho thấy ngưỡng tin cậy $\tau = 0{,}6$ hoạt động hiệu quả mà không từ chối quá nhiều dự đoán hợp lệ.

\section{Phân tích và Thảo luận Kết quả}
\label{sec:results_analysis}

\subsection{Vai trò của Đặc trưng Bất biến Hướng}

Toàn bộ thực nghiệm sử dụng loại đặc trưng \texttt{orientation\_invariant} (63 đặc trưng tính trên biên độ tổng hợp), thay vì \texttt{time\_domain} (90 đặc trưng trên từng trục riêng lẻ). Mặc dù số chiều ít hơn, kết quả cho thấy 63 đặc trưng bất biến hướng đủ để đạt 97--99\% trên bài toán 5 lớp.

Lợi thế chính: tính toán trên biên độ tổng hợp ($\text{acc\_mag} = \sqrt{a_X^2 + a_Y^2 + a_Z^2}$) loại bỏ phụ thuộc vào hướng gắn thiết bị --- mô hình vẫn hoạt động khi thiết bị đặt trong túi quần, đeo trên cổ tay, hay gắn ở thắt lưng. Đây là yêu cầu quan trọng cho ứng dụng thực tế nơi hướng gắn không thể kiểm soát.

\subsection{Tốc độ Huấn luyện và Quy trình Phát triển}

Thời gian huấn luyện cực ngắn (0,09--0,42 giây cho ML cổ điển, 1,94--10,09 giây cho PyTorch) cho phép quy trình phát triển lặp lại nhanh: người dùng có thể thu thập thêm dữ liệu, điều chỉnh tham số và huấn luyện lại trong vài giây. Đây là ưu thế đáng kể so với các pipeline huấn luyện nặng, đặc biệt trong bối cảnh giáo dục và nghiên cứu nơi tài nguyên tính toán hạn chế.

\subsection{Mô hình End-to-end so với Đặc trưng Thủ công}

PyTorch 1D-CNN đạt hiệu suất ngang PyTorch MLP (99,13\%) mặc dù hoạt động trên dữ liệu thô $150 \times 6$ mà không cần trích xuất đặc trưng thủ công. Tuy nhiên, các mô hình dựa trên đặc trưng thủ công có hai ưu thế cho triển khai nhúng: (1) vector đặc trưng 63 chiều nhỏ hơn đáng kể so với cửa sổ thô $150 \times 6 = 900$ giá trị, giảm yêu cầu bộ nhớ đầu vào; (2) mã suy luận cho mô hình cổ điển (phép nhân ma trận, so sánh cây quyết định) đơn giản và dễ xác minh hơn mã tích chập 1D.

\section{So sánh Định tính với Nền tảng Thương mại}
\label{sec:qualitative_comparison}

Bảng~\ref{tab:qualitative_comparison} so sánh các đặc điểm kiến trúc và chức năng giữa framework đề xuất và hai nền tảng thương mại phổ biến. So sánh tập trung vào các thuộc tính có thể xác minh khách quan, không so sánh số liệu hiệu suất do sự khác biệt về tập dữ liệu và cấu hình.

\begin{table}[H]
    \centering
    \caption{So sánh định tính với các nền tảng thương mại}
    ...
    \begin{tabular}{@{}p{3.5cm}p{3.5cm}p{3.5cm}p{3.5cm}@{}}
        ...
        Xác minh tương đồng & Có (Python $\leftrightarrow$ C++) & Không công khai & Không công khai \\
        Ngưỡng tin cậy tích hợp & Có (trong mã suy luận) & Anomaly block riêng & Không tích hợp \\
        Tăng cường nhận biết lớp & Có & Không & Không \\
        \bottomrule
    \end{tabular}
\end{table}

Điểm phân biệt quan trọng nhất là \textbf{tính minh bạch}: mã C++ được sinh ra có thể đọc, kiểm tra và sửa đổi trực tiếp. Trong quá trình phát triển, chính tính minh bạch này đã cho phép phát hiện và sửa các lỗi tương đồng huấn luyện-triển khai (lỗi zero-padding, sai lệch công thức kurtosis/skewness, sai thứ tự đặc trưng --- xem Mục~\ref{sec:training_deployment_parity}) mà các nền tảng hộp đen không thể phát hiện. Phân tích chi tiết hơn về ý nghĩa kiến trúc được trình bày trong Chương~\ref{chap:discussion}.
...
\begin{enumerate}
    \item \textbf{Hiệu suất phân loại cao}: 97--99\% trên bài toán 5 lớp HAR với 63 đặc trưng bất biến hướng, đạt 100\% trên bài toán 3 lớp cơ bản.
    \item \textbf{Đa thuật toán}: Năm thuật toán (RF, SVM, NN, PyTorch MLP, PyTorch 1D-CNN) đều đạt hiệu suất cạnh tranh, với biên độ chênh lệch chỉ 1,75 điểm phần trăm --- cho phép lựa chọn theo ràng buộc triển khai thay vì chỉ theo độ chính xác.
    \item \textbf{Triển khai đa thiết bị}: Kiến trúc tạo mã hỗ trợ 5 nhóm nền tảng đích từ cùng một mô hình đã huấn luyện.
    \item \textbf{Huấn luyện nhanh}: Dưới 1 giây cho ML cổ điển, dưới 11 giây cho học sâu, phù hợp cho phát triển lặp lại.
    \item \textbf{Đặc trưng bất biến hướng hiệu quả}: 63 đặc trưng tính trên biên độ tổng hợp đạt hiệu suất cao mà không phụ thuộc hướng gắn thiết bị.
\end{enumerate}
