...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lllp{5.5cm}@{}}
        ...
        Neural Network & Treo & Treo & Suy luận float32 quá chậm, thiết bị không phản hồi \\
        SVM (RBF) & Lỗi & Lỗi & Không hoạt động trên cả hai cấu hình \\
        \bottomrule
    \end{tabular}
\end{table}

Kết quả cho thấy sự khác biệt rõ rệt giữa hiệu suất offline (97--100\% trên tập kiểm tra) và hiệu suất triển khai thực tế. Chỉ hai trong năm thuật toán hoạt động đáng tin cậy trên thiết bị: PyTorch 1D-CNN (cả hai cấu hình) và Random Forest (bài toán 3 lớp).
...
\textbf{Neural Network (sklearn MLP)} gặp hiện tượng treo: thiết bị không phản hồi sau khi bắt đầu suy luận. Framework hiện chưa hỗ trợ lượng tử hóa INT8 cho mô hình sklearn --- mã C++ sinh ra sử dụng số học float32 đầy đủ cho toàn bộ phép nhân ma trận. Với kiến trúc 63--100--50--5 (tổng cộng 11.550 trọng số), khối lượng tính toán float32 trên vi điều khiển 64~MHz có thể vượt quá khung thời gian cho phép, gây treo hệ thống.

\textbf{SVM (RBF)} không hoạt động trên thiết bị cho cả hai cấu hình. Kernel RBF yêu cầu tính khoảng cách Euclid đến mỗi support vector trong mỗi chu kỳ suy luận --- số lượng support vectors có thể lớn (hàng trăm) với tập huấn luyện 1063 mẫu, tạo ra khối lượng tính toán và yêu cầu bộ nhớ vượt quá khả năng của vi điều khiển.
...
\begin{enumerate}
    ...
    \item \textbf{Ràng buộc tài nguyên}: SVM (nhiều support vectors) và NN không lượng tử hóa vượt quá năng lực tính toán của vi điều khiển 64~MHz. Bài toán lựa chọn mô hình cho triển khai biên không chỉ dựa trên độ chính xác mà còn trên khối lượng tính toán suy luận.
...
\textbf{Khuyến nghị thực tế}: Dựa trên kết quả, PyTorch 1D-CNN là lựa chọn triển khai đáng tin cậy nhất cho cả bài toán đơn giản và phức tạp. Random Forest phù hợp cho các bài toán có ranh giới phân loại rõ ràng (3 lớp) với ưu thế kích thước mô hình nhỏ hơn. Các thuật toán còn lại cần cải thiện ở tầng code generation (lượng tử hóa INT8 cho NN, tối ưu hóa số support vectors cho SVM, xác minh weight export cho PyTorch MLP) trước khi triển khai đáng tin cậy.
...
\begin{enumerate}
    \item \textbf{Hiệu suất phân loại cao (offline)}: 97--99\% trên bài toán 5 lớp HAR với 63 đặc trưng bất biến hướng, đạt 100\% trên bài toán 3 lớp cơ bản.
    \item \textbf{Đa thuật toán}: Năm thuật toán (RF, SVM, NN, PyTorch MLP, PyTorch 1D-CNN) đều đạt hiệu suất cạnh tranh offline, với biên độ chênh lệch chỉ 1,75 điểm phần trăm.
    \item \textbf{Khoảng cách triển khai}: Chỉ PyTorch 1D-CNN hoạt động tốt trên thiết bị ở cả hai cấu hình; Random Forest hoạt động ổn định cho bài toán 3 lớp. Kết quả cho thấy độ chính xác offline không đảm bảo triển khai thành công --- lựa chọn mô hình cho thiết bị biên cần xét đến tương đồng tính toán, ràng buộc tài nguyên và điều kiện thực tế.
