...
Framework được triển khai dưới dạng ứng dụng web một trang sử dụng Python 3.10 làm ngôn ngữ chính, với Dash 2.14 (dựa trên Flask và Plotly) cho giao diện tương tác, scikit-learn 1.3 cho các thuật toán ML cổ điển (Random Forest, SVM, MLP), và PyTorch 2.x cho huấn luyện mô hình học sâu (MLP, CNN). Xử lý dữ liệu dựa trên NumPy và Pandas; trực quan hóa tương tác sử dụng Plotly với hỗ trợ kéo thả; và Jinja2 được dùng làm template engine để sinh mã C/C++ từ các mẫu tham số hóa.

Nền tảng chuẩn triển khai là Seeed XIAO nRF52840 Sense (ARM Cortex-M4F @ 64MHz, 256KB RAM, 1MB Flash, IMU LSM6DS3 tích hợp), được chọn vì: tích hợp đầy đủ IMU 6 trục, hiệu năng đại diện cho phân khúc MCU trung bình, hỗ trợ Arduino và PlatformIO, và giá thành thấp (\textasciitilde\$10 USD). Biên dịch chéo sử dụng GCC ARM Embedded Toolchain qua Arduino IDE 2.3 hoặc PlatformIO.
...
Framework hỗ trợ 6 chế độ trích xuất đặc trưng (chi tiết tại Bảng~\ref{tab:feature_modes}, Chương~\ref{chap:methodology}), từ đặc trưng bất biến hướng trên magnitude vectors (33--63 chiều, phù hợp triển khai nhúng) đến đặc trưng per-axis miền thời gian và tần số (66--156 chiều), cùng chế độ raw cho đầu vào CNN. Giao diện cho phép người dùng chọn chế độ và xem trước số lượng đặc trưng trước khi trích xuất.

\paragraph{Cơ sở chọn đặc trưng bất biến hướng}
Chế độ \texttt{orientation\_invariant} chỉ sử dụng các đặc trưng tính trên magnitude vectors ($\sqrt{x^2+y^2+z^2}$) thay vì per-axis features. Điều này đảm bảo mô hình không bị ảnh hưởng bởi định hướng lắp đặt cảm biến: đeo thiết bị ngang hoặc dọc, xoay 90° vẫn cho cùng một magnitude. Tuy nhiên, trade-off là mất thông tin về hướng chuyển động (VD: leo cầu thang khó phân biệt với xuống cầu thang chỉ dựa trên magnitude).

\subsection{Tích hợp tăng cường dữ liệu}
\label{subsec:data_augmentation_integration}

Module tích hợp năm phương pháp tăng cường dữ liệu IMU đã trình bày trong Mục~\ref{sec:data_augmentation} (jitter, rotation, scaling, time warping, permutation) với chiến lược nhận biết lớp: hoạt động tĩnh chỉ nhận micro-jitter, hoạt động động nhận tất cả biến đổi. Giao diện cho phép kích hoạt/tắt từng phương pháp riêng biệt, điều chỉnh hệ số tăng cường (1$\times$--3$\times$), và xem phân phối lớp trước/sau tăng cường.
