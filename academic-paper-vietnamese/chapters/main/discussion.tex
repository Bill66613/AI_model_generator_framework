...
Kết quả triển khai trên thiết bị (Bảng~\ref{tab:deployment_status}) cho thấy một phát hiện quan trọng: chỉ PyTorch 1D-CNN hoạt động đáng tin cậy trên cả hai cấu hình bài toán (3 lớp và 5 lớp), trong khi Random Forest chỉ ổn định cho bài toán 3 lớp. Neural Network và PyTorch MLP hoạt động một phần nhưng gặp nhầm lẫn lớp có hệ thống. SVM bị loại khỏi đánh giá do nghi ngờ lỗi tầng tạo mã.

Phát hiện này bổ sung chiều sâu cho kết luận về lựa chọn thuật toán: biên độ chênh lệch 1,75 điểm phần trăm trên tập kiểm tra offline trở nên không có ý nghĩa khi một số mô hình gặp nhầm lẫn lớp nghiêm trọng trên thiết bị. Tiêu chí lựa chọn mô hình cho triển khai biên cần mở rộng từ ``độ chính xác cao nhất'' sang bao gồm \textit{khả năng triển khai thực tế}---bao gồm độ chính xác số học, khối lượng tính toán suy luận, và tính ổn định của pipeline code generation.

Đáng chú ý, CNN --- mô hình end-to-end hoạt động trên dữ liệu thô --- hoạt động tốt nhất trên thiết bị, trong khi các mô hình dựa trên đặc trưng thủ công gặp nhiều vấn đề hơn. Điều này gợi ý rằng việc loại bỏ bước trích xuất đặc trưng trong mã C++ (nguồn chính của sai lệch tương đồng) mang lại lợi thế triển khai đáng kể, dù với chi phí mô hình lớn hơn và tính diễn giải thấp hơn.
...
\textbf{SensiML} nhắm mục tiêu triển khai IoT thương mại với AutoML tinh vi nhưng chi phí \$99--500/tháng.

Các điểm phân biệt kiến trúc chính nằm ở tính minh bạch mã sinh ra, hỗ trợ thuật toán ML cổ điển, và khả năng xác minh tương đồng Python$\leftrightarrow$C++ (xem Bảng~\ref{tab:qualitative_comparison} trong Chương~\ref{chap:results}).
...
Tuy nhiên, kết quả thực nghiệm cho thấy PyTorch MLP và 1D-CNN đạt hiệu suất cao nhất (99,13\% trên 5 lớp), và quan trọng hơn, \textit{chỉ 1D-CNN hoạt động đáng tin cậy trên thiết bị cho cả hai cấu hình bài toán} (Bảng~\ref{tab:deployment_status}). Điều này xác nhận giá trị của cả hai hướng tiếp cận: ML cổ điển cho prototyping nhanh và các bài toán đơn giản (RF hoạt động tốt cho 3 lớp), học sâu end-to-end cho triển khai đáng tin cậy trên các bài toán phức tạp.
