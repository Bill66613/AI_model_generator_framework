...
Kết quả triển khai trên thiết bị (Bảng~\ref{tab:deployment_status}) cho thấy một phát hiện quan trọng: chỉ PyTorch 1D-CNN hoạt động đáng tin cậy trên cả hai cấu hình bài toán (3 lớp và 5 lớp), trong khi Random Forest chỉ ổn định cho bài toán 3 lớp. Ba thuật toán còn lại (Neural Network sklearn, SVM, PyTorch MLP) gặp các vấn đề khác nhau --- từ treo hệ thống đến nhầm lẫn lớp có hệ thống.

Phát hiện này bổ sung chiều sâu cho kết luận về lựa chọn thuật toán: biên độ chênh lệch 1,75 điểm phần trăm trên tập kiểm tra offline trở nên không có ý nghĩa khi một số mô hình hoàn toàn không hoạt động trên thiết bị. Tiêu chí lựa chọn mô hình cho triển khai biên cần mở rộng từ ``độ chính xác cao nhất'' sang bao gồm \textit{khả năng triển khai thực tế}---bao gồm khối lượng tính toán suy luận, yêu cầu bộ nhớ, và tính ổn định của pipeline code generation.
