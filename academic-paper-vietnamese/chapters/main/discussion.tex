...
Sử dụng bộ nhớ (95-182KB flash) tương tự bảo thủ, chiếm 9-18\% của 1MB khả dụng trên Seeed XIAO nRF52840. Điều này cho phép triển khai trên các nền tảng bị giới hạn hơn như Arduino Nano 33 BLE (256KB flash) hoặc ESP32-C3 (384KB flash), mở rộng khả năng áp dụng.
...
Kết luận: framework của chúng tôi chiếm một vị thế độc đáo kết hợp hiệu suất cấp chuyên nghiệp, tính minh bạch đầy đủ và chi phí bằng không—một kết hợp không có sẵn trong các giải pháp hiện có.
...
Trong khi chúng tôi chứng minh triển khai trên ARM Cortex-M4 (Seeed XIAO), framework tuyên bố tạo mã không phụ thuộc nền tảng. Xác thực trên các kiến trúc khác—AVR (Arduino Uno), RISC-V (ESP32-C3), ARM Cortex-M0+ (Raspberry Pi Pico)—là cần thiết để xác nhận tính di động. Đặc điểm bộ nhớ và tốc độ sẽ khác nhau, đặc biệt trên các bộ xử lý AVR 8-bit thiếu hỗ trợ dấu phẩy động phần cứng.
...
Bộ 138 đặc trưng của chúng tôi sử dụng các thống kê miền thời gian/tần số thủ công. Điều này yêu cầu kiến thức miền (biết đặc trưng nào phân biệt các hoạt động) nhưng đảm bảo khả năng diễn giải và biểu diễn gọn nhẹ. Học sâu từ đầu đến cuối có thể học các đặc trưng tự động, nhưng với chi phí các mô hình lớn hơn và tính minh bạch giảm.
...
Thứ nhất, \textit{dấu chân bộ nhớ tối thiểu}: Neural Network chỉ chiếm 95KB flash---bao gồm trọng số mô hình, logic suy luận, trích xuất đặc trưng 15 thống kê, và chuẩn hóa StandardScaler. Với TFLite Micro, cùng chức năng sẽ yêu cầu thêm 150--250KB cho runtime interpreter, đẩy tổng lên 245--345KB và chiếm $>$25\% flash khả dụng chỉ cho suy luận. Trên Seeed XIAO (1MB flash), điều này vẫn khả thi; trên các vi điều khiển với 256KB flash (ví dụ: STM32F103, ATmega2560), phương pháp runtime sẽ không thể triển khai.
...
Khi triển khai trên thiết bị Seeed XIAO nRF52840, các lỗi không khớp khiến mô hình luôn dự đoán lớp \texttt{walking\_downstairs} bất kể hoạt động thực tế. Phân tích cho thấy output biases của mạng nơ-ron $[-0{,}116; -0{,}160; -0{,}066; 0{,}116; 0{,}048]$ chi phối khi đặc trưng đầu vào nằm hoàn toàn ngoài phân phối huấn luyện---lớp có bias cao nhất ($0{,}116$, tương ứng \texttt{walking\_downstairs}) luôn thắng quyết.
