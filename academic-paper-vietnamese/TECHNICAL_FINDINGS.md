...
**Version:** 9.0 (prepare merge of PR#3: CNN metadata fix, Kalman filter parity, TFLite deployment scope)
...
| 15 | TFLite deployment scope: RF/SVM not convertible via onnx2tf | DESIGN | ⚠️ LIMITATION | (PR#3) deployment tooling | Ch.2 §related work, Ch.5 §limitations |
| 16 | CNN code generation metadata mismatch — wrong feature/channel count | MEDIUM | ✅ FIXED | (PR#3) `callbacks/code_generation_callbacks.py`, `deployment/cnn_generator.py` | Ch.3 §CodeGen, Ch.5 §multi-arch |
| 17 | Kalman filter (causal) preprocessing with exact deployment parity | FEATURE | ✅ IMPLEMENTED | (PR#3) `utils/data_processing.py`, `deployment/base_generator.py`, `callbacks/preprocessing_callbacks.py` | Ch.3 §preprocessing, Ch.5 §parity |
...
- **Finding 15 (TFLite scope)** → Evidence: PR#3 notes that RF/SVM rely on ONNX-ML ops (TreeEnsembleClassifier) not supported by onnx2tf; only NN/CNN are feasible conversion targets → Thesis: Ch.5 limitations + comparison
- **Finding 16 (CNN metadata mismatch)** → Code: `callbacks/code_generation_callbacks.py` and `deployment/cnn_generator.py` adjustments to keep displayed/generated metadata consistent → Thesis: Ch.3 multi-architecture support, Ch.5 pipeline correctness
- **Finding 17 (Kalman filter)** → Code: `utils/data_processing.py` (Python Kalman), `deployment/base_generator.py` (C++ generation), `callbacks/preprocessing_callbacks.py` (UI wiring) → Thesis: Ch.3 preprocessing, Ch.5 training-deployment parity
