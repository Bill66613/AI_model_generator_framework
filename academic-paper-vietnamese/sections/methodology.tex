...
For each window and each sensor axis (6 axes total: 3 accelerometer + 3 gyroscope), the framework extracts 15 time-domain statistical features. \textit{Note: The ``6'' refers to sensor axes from the 6-axis IMU, not the number of activity classes, which is 5.} Frequency-domain features were intentionally excluded after identifying training-deployment parity issues: FFT implementations differ substantially between Python (NumPy) and embedded C++ (approximations or ARM CMSIS-DSP), leading to feature value discrepancies that degrade deployed model accuracy.
