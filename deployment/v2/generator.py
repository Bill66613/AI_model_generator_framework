...
class HARCodeGenerator:
    ...
    def __init__(
        ...
        self.feature_names: List[str] = [str(f) for f in model_data.get("feature_names", [])]
        self.classes: List[str] = [str(c) for c in model_data.get("classes", [])]
        self.feature_means: List[float] = model_data.get("feature_means", [0.0] * len(self.feature_names))
        self.feature_stds: List[float] = model_data.get("feature_stds", [1.0] * len(self.feature_names))
        ...
        sampling_rate = fe_cfg.get("sampling_rate") or model_params.get("sampling_rate", 100)
        window_ms = fe_cfg.get("window_size_ms") or model_params.get("window_size_ms", 1500)
        self.sampling_rate: int = int(sampling_rate)
        self.window_size: int = int(round(window_ms / 1000 * self.sampling_rate))
        self.preprocessing: Dict = fe_cfg.get("preprocessing") or {}

        # --- Derived precision ---
        self.precision = {"accuracy": 6, "balanced": 4, "speed": 3, "power": 3}.get(optimization, 4)
        ...
        self.sketch_name = _safe_c_ident(model_data.get('model_name', 'har_sketch')).lower()
    ...
    def generate_files(self) -> Dict[str, str]:
        ...
        sketch_content = self._sketch.generate()
        files[f"{self.sketch_name}.ino"] = sketch_content
