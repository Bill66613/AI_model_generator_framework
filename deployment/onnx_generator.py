...
class ONNXRuntimeCodeGenerator(BaseCodeGenerator):
    ...
    def _get_model_specific_declarations(self) -> str:
        ...
        lines.append("")
        lines.append(
            "// ONNX model embedded as byte array (for MCU deployment)")
        ...
        lines.append(
            "bool onnx_init_from_buffer(const unsigned char* buffer, unsigned int size);")
        lines.append(
            f"int onnx_predict(float features[NUM_FEATURES], float probabilities[NUM_CLASSES]);")
    ...
    def _generate_onnx_c_array(self) -> str:
        ...
        if self._onnx_info:
            lines.append(
                f"// ONNX opset: {self._onnx_info.get('opset_version', 'unknown')}")
            lines.append(
                f"// Parameters: {self._onnx_info.get('total_parameters', 'unknown')}")
        lines.append(f"")
    ...
    def _generate_prediction_function(self) -> str:
        ...
        lines.append(
            "// 1. File-based: Load .onnx from filesystem (Linux/RPi/Jetson)")
    ...
    def _generate_mcu_onnx_prediction(self, lines: list) -> str:
        ...
        lines.append(
            "//   3. ONNX Runtime Micro (if available for your platform)")
        lines.append("//")
        lines.append(
            "// This code provides the ONNX model as a byte array that can be")
        lines.append(
            "// used with any compatible runtime on your target platform.")
        lines.append("")
        lines.append(
            "// Placeholder inference — replace with your runtime's API")
        lines.append(f"static bool onnx_initialized = false;")
        lines.append("")
        lines.append(
            "bool onnx_init_from_buffer(const unsigned char* buffer, unsigned int size) {")
        lines.append(
            "    // Initialize your ONNX runtime here with the model buffer")
        lines.append(
            "    // For ONNX Runtime Micro: OrtCreateSessionFromArray(buffer, size, ...)")
        ...
        lines.append("bool onnx_init(const char* model_path) {")
        lines.append(
            "    // Load model from filesystem (not typically available on MCU)")
        lines.append(
            "    return onnx_init_from_buffer(g_onnx_model, g_onnx_model_len);")
        lines.append("}")
        lines.append("")
        lines.append(
            f"int onnx_predict(float features[NUM_FEATURES], float probabilities[NUM_CLASSES]) {{")
        ...
        lines.append(
            "        if (!onnx_init_from_buffer(g_onnx_model, g_onnx_model_len)) return -1;")
        lines.append("    }")
        lines.append("")
        lines.append(
            "    // TODO: Replace with actual ONNX Runtime Micro inference call")
        lines.append("    // Example for ORT Micro:")
        lines.append(
            "    // OrtValue* input = OrtCreateTensorWithData(features, NUM_FEATURES * sizeof(float));")
        lines.append("    // OrtValue* output = nullptr;")
        lines.append("    // OrtRun(session, &input, 1, &output, 1);")
        lines.append(
            "    // memcpy(probabilities, OrtGetTensorData(output), NUM_CLASSES * sizeof(float));")
        lines.append("")
        ...
        lines.append("")
        lines.append(
            "// har_predict_internal: bridges base har_predict() → onnx_predict()")
        lines.append(
    ...
    def _generate_full_onnx_prediction(self, lines: list) -> str:
        ...
        lines.append("    try {")
        lines.append(
            "        ort_env = new Ort::Env(ORT_LOGGING_LEVEL_WARNING, \"har_inference\");")
        ...
        lines.append("")
        lines.append(
            "        HAR_LOG(\"ONNX Runtime initialized: %s\", model_path);")
        ...
        lines.append("")
        lines.append(
            "        ort_session = new Ort::Session(*ort_env, buffer, size, session_options);")
        lines.append("        ort_memory_info = new Ort::MemoryInfo(")
        lines.append(
            "            Ort::MemoryInfo::CreateCpu(OrtArenaAllocator, OrtMemTypeDefault));")
        lines.append("")
        lines.append(
            "        HAR_LOG(\"ONNX Runtime initialized from buffer (%u bytes)\", size);")
        lines.append("        return true;")
        lines.append("    } catch (const Ort::Exception& e) {")
        lines.append("        HAR_LOG(\"ONNX Runtime error: %s\", e.what());")
        lines.append("        return false;")
        ...
        lines.append(
            f"        std::array<int64_t, 2> input_shape = {{1, NUM_FEATURES}};")
        lines.append(
        ...
        lines.append("bool onnx_init(const char* model_path) {")
        lines.append(
            "    HAR_LOG(\"ONNX Runtime not available. Define USE_ONNX_RUNTIME to enable.\");")
        lines.append("    return false;")
        lines.append("}")
        lines.append("")
        lines.append(
    ...
    def _generate_utility_functions(self) -> str:
        ...
        lines.append(
            f"    HAR_LOG(\"Model: ONNX Runtime ({self.model_type})\");")
        lines.append(
            f"    HAR_LOG(\"Model size: %u bytes\", g_onnx_model_len);")
        lines.append(
            f"    HAR_LOG(\"Features: %d, Classes: %d\", NUM_FEATURES, NUM_CLASSES);")
        if self._onnx_info:
            lines.append(
                f"    HAR_LOG(\"ONNX opset: {self._onnx_info.get('opset_version', '?')}\");")
            lines.append(
                f"    HAR_LOG(\"Parameters: {self._onnx_info.get('total_parameters', '?')}\");")
        lines.append("}")
    ...
    def generate_example_sketch(self, header_filename: str = None) -> str:
        ...
        sketch.append(
            f" *   - Define USE_ONNX_RUNTIME before including headers")
        sketch.append(f" *")
        sketch.append(f" * For MCU deployment:")
        sketch.append(
            f" *   - The ONNX model is embedded as a byte array in the code")
    ...
    def _generate_arduino_onnx_sketch(self, sketch: list, header_filename: str) -> str:
        ...
        sketch.append(
            f"    Serial.println(\"HAR ONNX Runtime - Initializing...\");")
        sketch.append(f"")
        ...
        sketch.append(
            f"    if (!onnx_init_from_buffer(g_onnx_model, g_onnx_model_len)) {{")
        sketch.append(f"        Serial.println(\"ERROR: ONNX init failed!\");")
        sketch.append(
            f"        while (1) {{ delay(2000); Serial.println(\"ONNX init failed\"); }}")
        sketch.append(f"    }}")
    ...
    def _generate_linux_edge_sketch(self, sketch: list, header_filename: str) -> str:
        ...
        sketch.append(f"    float sensor_buffer[WINDOW_SIZE][N_CHANNELS];")
        sketch.append(
            f"    // TODO: Fill sensor_buffer with real data from your IMU/sensor")
        sketch.append(f"")
        ...
        sketch.append(
            f"        printf(\"Predicted: %s (confidence: %.4f)\\n\",")
        sketch.append(
            f"               get_activity_name(predicted), confidence);")
        sketch.append(f"    }} else {{")
        sketch.append(
            f"        printf(\"Unknown activity (confidence: %.4f)\\n\", confidence);")
        sketch.append(f"    }}")
        sketch.append(f"")
    ...
    def _generate_placeholder_model(self, error_msg: str) -> str:
        ...
        lines.append(f"// ========================================")
        lines.append(
            f"// ONNX conversion requires 'onnx' and 'skl2onnx' (sklearn) or 'torch' (PyTorch).")
        lines.append(f"// Install with: pip install onnx skl2onnx")
        lines.append(f"// Error: {error_msg}")
        lines.append(f"//")
        lines.append(
            f"// After installing, regenerate the code to embed the actual ONNX model.")
        lines.append(f"// ========================================")
        lines.append(f"")
        lines.append(
            f"alignas(16) const unsigned char g_onnx_model[] = {{0x00}};")
        lines.append(f"const unsigned int g_onnx_model_len = 0;")
        lines.append(f"")
        lines.append(
            f"#warning \"ONNX model not converted — install onnx/skl2onnx and regenerate\"")
        return '\n'.join(lines)
