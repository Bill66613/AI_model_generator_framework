    ...
    try:
        ...
        for df_window, label in zip(padded_windows, all_labels):
            if feature_method == 'orientation_invariant':
                # Base features from original window
                features = _compute_orientation_invariant_features(df_window)
                features['activity'] = label
                feature_list.append(features)

                # Data augmentation: add a few rotated variants
                AUGMENTATIONS = 3
                for _ in range(AUGMENTATIONS):
                    df_rot = _apply_rotation(df_window)
                    feats_aug = _compute_orientation_invariant_features(df_rot)
                    feats_aug['activity'] = label
                    feature_list.append(feats_aug)

            else:
                # Extract time-domain features
                time_features = {}
                if feature_method in ['all', 'time_domain']:
                    time_df = extract_time_domain_features(
                        df_window, sensor_cols)
                    time_features = time_df.iloc[0].to_dict()

                # Extract frequency-domain features
                freq_features = {}
                if feature_method in ['all', 'frequency_domain']:
                    freq_df = extract_frequency_domain_features(
                        df_window, sensor_cols, sampling_rate=sampling_rate)
