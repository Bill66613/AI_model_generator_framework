...
Data collection utilizes the Seeed XIAO nRF52840 Sense microcontroller \cite{web:XIAO}, featuring an ARM Cortex-M4F processor (64MHz, 256KB RAM, 1MB Flash) and an integrated LSM6DS3 6-axis IMU sensor communicating via I2C at address 0x6A. The LSM6DS3 provides 3-axis accelerometer data (configured at ±2g range) and 3-axis gyroscope data, sampled at 100Hz to ensure sufficient temporal resolution for capturing human motion dynamics while balancing memory constraints.
...
\begin{enumerate}
    ...
    \item Configure accelerometer (±2g range) and gyroscope sensors
    ...
    \item Output CSV format: Time\_seconds, AccX, AccY, AccZ, GyrX, GyrY, GyrZ
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Outlier Removal}: Identifies and removes samples exceeding specified standard deviation thresholds (default: 3σ), addressing sensor anomalies and electromagnetic interference artifacts.
...
Preprocessed signals are segmented into fixed-size overlapping windows for feature extraction. Default configuration:
\begin{itemize}
    \item \textbf{Window Size}: 75 samples (0.75 seconds at 100Hz)
    \item \textbf{Overlap}: 50\% (37-sample stride)
\end{itemize}
This balances temporal context (capturing full motion cycles) with responsiveness (enabling sub-second inference updates).

\subsubsection{Feature Set}
For each window and each sensor axis (6 axes total), the framework computes 23 statistical features spanning time and frequency domains:

\textbf{Time-Domain Features:}
\begin{itemize}
    \item Mean, Standard Deviation, Minimum, Maximum, Range
    \item Root Mean Square (RMS): $\text{RMS} = \sqrt{\frac{1}{N}\sum_{i=1}^{N} x_i^2}$
    \item Median Absolute Deviation: $\text{MAD} = \text{median}(|x_i - \text{median}(x)|)$
    \item Skewness: $\gamma = \frac{E[(X-\mu)^3]}{\sigma^3}$
    \item Kurtosis: $\kappa = \frac{E[(X-\mu)^4]}{\sigma^4} - 3$
    \item Zero-Crossing Rate: count of signal sign changes
    \item Energy: $E = \sum_{i=1}^{N} x_i^2$
\end{itemize}

\textbf{Frequency-Domain Features:}
\begin{itemize}
    \item Dominant Frequency: frequency bin with maximum FFT magnitude
    \item Spectral Energy: sum of squared FFT coefficients
    \item Spectral Entropy: Shannon entropy of normalized power spectrum
\end{itemize}

Total feature vector dimensionality: $23 \times 6 = 138$ features per window. This comprehensive representation captures both motion amplitude/dynamics (time-domain) and periodicity/frequency characteristics (frequency-domain), enabling discrimination of activities with similar amplitude profiles but different motion patterns.
...
\begin{itemize}
    \item Architecture: 138 input → 100 hidden → num\_classes output
    \item Activation: ReLU (hidden), Softmax (output)
    \item Optimizer: Adam with learning rate 0.001
    \item Max iterations: 500 epochs
\end{itemize}
...
\begin{itemize}
    ...
    \item Comprehensive feature set (138 features)
\end{itemize}
...
\begin{itemize}
    ...
    \item Selective feature set (top 80-100 features by importance)
...
Validation experiments use a Human Activity Recognition (HAR) dataset collected from a single subject performing six activities:
\begin{itemize}
    \item \textit{Standing}, \textit{Still} (sitting), \textit{Walking}, \textit{Walking Upstairs}, \textit{Walking Downstairs}, \textit{Running}
\end{itemize}
Data was collected using the Seeed XIAO nRF52840 Sense platform at 100Hz over multiple recording sessions. Total dataset size: approximately 2770 samples (27.7 seconds) per activity, yielding $\sim$16,620 samples across all classes. Initial analysis revealed class imbalance with \textit{standing} comprising 35\% of samples and \textit{walking\_downstairs} only 9\%, an imbalance ratio of 3.8:1 necessitating class balancing techniques.
...
\begin{itemize}
    \item \textbf{Inference Time}: Average time to classify one window (ms)
    \item \textbf{Memory Usage}: Flash (program storage) and RAM (runtime) consumption (KB)
    \item \textbf{Power Consumption}: Average current draw during inference (mA)
\end{itemize}
