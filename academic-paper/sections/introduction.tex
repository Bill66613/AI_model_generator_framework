...
\subsubsection{Signal Processing Principles}
...
\subsubsection{Feature Extraction Rationale}

Raw sensor data, while information-rich, contains redundancy and noise that hinder classification performance. Feature extraction transforms high-dimensional time-series data into compact statistical representations that emphasize discriminative characteristics. Time-domain features (mean, variance, skewness, kurtosis) capture distribution properties, while frequency-domain features derived from Fourier transforms reveal periodic patterns \cite{oppenheim1999discrete}. This dimensionality reduction is essential for embedded deployment, as computing predictions on 90 carefully chosen features (15 statistical measures per sensor axis × 6 IMU axes) requires orders of magnitude less memory and computation than processing 450 raw samples (75 samples × 6 sensor axes) directly.
...
\subsubsection{Biomechanical Basis for Activity Recognition}
...
\subsubsection{Interactive Preprocessing Approach}
...
\subsubsection{Machine Learning Algorithm Selection}
