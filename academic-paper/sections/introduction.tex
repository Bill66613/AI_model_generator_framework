...
\midrule
\textbf{Optimization Modes} & Accuracy/latency trade-off & AutoML optimization & Manual quantization & 4 modes (accuracy, speed, power, balanced) \\
\midrule
\textbf{Class Imbalance} & Manual oversampling & Automated balancing & No built-in support & Automatic (class\_weight) \\
\midrule
\textbf{Customization} & Limited (closed platform) & Moderate (proprietary) & High (open source) & High (modular design) \\
\midrule
\textbf{Educational Use} & Free tier (limited) & Academic license & Yes & Yes (no restrictions) \\
\midrule
