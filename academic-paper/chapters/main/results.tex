...
All activities achieve F1-scores above 0.93, indicating balanced precision-recall performance. \textit{Running} is most accurately detected (F1 = 0.98), likely due to distinctive high-intensity acceleration patterns. \textit{Walking Downstairs} achieves 0.97 F1-score despite being challenging to distinguish from regular walking. \textit{Walking Upstairs} shows slightly lower recall (0.93), with occasional confusion with regular \textit{Walking} (see confusion matrix analysis below).
...
\begin{itemize}
    ...
    \item \textbf{Walking vs Walking Downstairs}: Minor confusion (2.3\%) between regular walking and downstairs walking, as both involve similar gait patterns
    \item \textbf{Running}: Zero confusion with other classes, indicating highly distinctive high-intensity motion signature
    \item \textbf{Still}: Well-separated from motion activities (>95\% precision/recall)
\end{itemize}
...
\\section{Optimization Mode Trade-offs}
...
\\section{Hyperparameter Optimization Impact}
...
\begin{enumerate}
    \item High accuracy (94-96\%) across three ML algorithms on challenging 5-class HAR task
